...
export async function resetPrivateEngineStorage(): Promise<PrivateEngineStorageResetReport> {
  try {
    ...
    if (typeof storage?.getDirectory !== 'function') {
      return { ok: false, removedEntries: 0, error: 'This browser does not expose OPFS cleanup APIs.' }
    }

    const root = await storage.getDirectory()
    if (typeof root.removeEntry !== 'function') {
      return { ok: false, removedEntries: 0, error: 'This browser cannot remove OPFS entries from the extension.' }
    }
