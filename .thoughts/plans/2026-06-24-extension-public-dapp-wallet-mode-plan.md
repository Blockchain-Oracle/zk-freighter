...
- Product spec: `.thoughts/specs/2026-06-22-zk-freighter-product-spec.md`, especially R11/A9.
- MVP stories: `.thoughts/stories/2026-06-22-zk-freighter-mvp-stories.md`, especially S17.
...
- It is opt-in from the user perspective. ZK Freighter stays privacy-by-default, and dApp mode is explained as public Stellar address/signing behavior.
- It reuses the seed-derived public Stellar account already used by ZK Freighter. Do not invent a second public signing key unless Abu explicitly changes the product decision.
...
- Stellar Wallets Kit compatibility is verified after direct Freighter-style messages pass. If Wallets Kit does not detect ZK Freighter cleanly, build a first-class ZK Freighter module/demo rather than pretending it is Freighter.
...
- Coexistence behavior when Freighter and ZK Freighter are both installed and a dApp sends Freighter-style page messages.
...
- `pnpm --filter @zk-freighter/extension typecheck`
- `pnpm test`
...
Use a real auth-entry fixture generated by Stellar SDK/Soroban simulation or current ZK Freighter prepared transaction code. Verify the returned signed auth entry locally where possible.
...
Prove how dApps should integrate ZK Freighter through Stellar Wallets Kit.

### Work

- Build a small local dApp harness using `@creit.tech/stellar-wallets-kit`.
- Test whether the Freighter module can discover/use ZK Freighter without conflict.
- If not, build a minimal ZK Freighter Wallets Kit module in the harness or docs, with `isAvailable`, `getAddress`, `signTransaction`, and later `signAuthEntry`.
- Keep `isAvailable()` below the Wallets Kit 1000 ms expectation.
- Keep branding honest: do not present ZK Freighter as Freighter.
